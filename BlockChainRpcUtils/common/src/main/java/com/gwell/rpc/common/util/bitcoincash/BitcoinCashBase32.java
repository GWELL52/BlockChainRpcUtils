...
public class BitcoinCashBase32 {
  ...
  /**
   * Encode a byte array as base32 string. This method assumes that all bytes are only from 0-31
   *
   * @param byteArray
   * @return
   */
    ...
    for (int i = 0; i < byteArray.length; i++) {
      ...
      if (val < 0 || val > 31) {
        throw new RuntimeException(
            "This method assumes that all bytes are only from 0-31. Was: " + val);
      }
