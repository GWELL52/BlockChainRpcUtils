...
public class BitcoinCashAddressFormatter {
  ...
  private static final BigInteger[] POLYMOD_GENERATORS =
      new BigInteger[] {
  ...
  public static String toCashAddress(
      BitcoinCashAddressType addressType, byte[] hash, MoneyNetwork network) {
    ...
    String cashAddress =
        BitcoinCashBase32.encode(concatenateByteArrays(payloadBytes, checksumBytes));
    return prefixString + SEPARATOR + cashAddress;
  }

  public static BitcoinCashAddressDecodedParts decodeCashAddress(
      String bitcoinCashAddress, MoneyNetwork network) {
  ...
  public static boolean isValidCashAddress(String bitcoinCashAddress, MoneyNetwork moneyNetwork) {
    try {
      ...
      byte[] checksumData =
          concatenateByteArrays(
              concatenateByteArrays(getPrefixBytes(prefix, moneyNetwork), new byte[] {0x00}),
  ...
  /**
   * @param checksumInput
   * @return Returns a 40 bits checksum in form of 5 8-bit arrays. This still has to me mapped to
  ...
  private static byte[] calculateChecksumBytesPolymod(byte[] checksumInput) {
    ...
    if (checksum.length == 5) {
      ...
      System.arraycopy(
          checksum,
          Math.max(0, checksum.length - 5),
          newChecksumArray,
          Math.max(0, 5 - checksum.length),
          Math.min(5, checksum.length));
  ...
  /**
   * Concatenates the two given byte arrays and returns the combined byte array.
   *
   * @param first
   * @param second
   * @return
